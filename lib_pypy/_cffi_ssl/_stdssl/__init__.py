...
HAS_KEYLOG = hasattr(lib, "SSL_CTX_set_keylog_callback")
...
class _SSLContext(object):
    ...
    if HAS_KEYLOG:
        ...
        def keylog_filename(self, arg):
            # In CPython, this is in _ssl/debughelpers.c
            # Reset variables and callback first
            lib.SSL_CTX_set_keylog_callback(self.ctx, ffi.NULL)
            if (self.keylog_bio != None):
                bio = self.keylog_bio
                self.keylog_bio = None
                lib.BIO_free_all(bio)
            if arg is None:
                self._keylog_filename = None
                return 0
            ...
            if lib.BIO_tell(self.keylog_bio) == 0:
                lib.BIO_puts(self.keylog_bio,
                         "# TLS secrets log file, generated by OpenSSL / Python\n".encode())
                lib.BIO_flush(self.keylog_bio)
            lib.SSL_CTX_set_keylog_callback(self.ctx, _keylog_callback)
            return 0
    ...
    def __del__(self):
        # In CPython this is done via context_clear
        if HAS_KEYLOG and self.keylog_bio:
            lib.BIO_free_all(self.keylog_bio)
            self.keylog_bio = None
...
keylog_callback_lock = threading.Lock()

@ffi.callback("void(SSL*, const char *)")
def _keylog_callback(ssl, line):
    ssl_obj = ffi.from_handle(lib.SSL_get_app_data(ssl))
    bio = ssl_obj.ctx.keylog_bio
    if not bio:
        return
    keylog_callback_lock.acquire(True)
    if isinstance(line, str):
        line = line.encode()
        buff = ffi.from_buffer(line)
    elif isinstance(line, bytes):
        buff = ffi.from_buffer(line)
    else:
        buff = line
    fmt = ffi.from_buffer(b"%s\n")
    res = lib.BIO_printf(bio, fmt, buff)
    e = ffi.errno
    lib.BIO_flush(bio)
    keylog_callback_lock.release()
    if res < 0:
        raise OSError(e, ssl_obj.ctx.keylog_filename)
